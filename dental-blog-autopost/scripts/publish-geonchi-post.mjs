// 건치 쇼츠 에피소드 하나를 블로그 글(영상+캡쳐+지식)로 발행한다.
// 사용법: node --env-file=.env.local scripts/publish-geonchi-post.mjs <html-file> "<제목>" "라벨1,라벨2"

import fs from "node:fs";

const [, , htmlFile, title, labelsArg] = process.argv;
if (!htmlFile || !title) {
  console.error('사용법: node --env-file=.env.local scripts/publish-geonchi-post.mjs <html-file> "<제목>" "라벨1,라벨2"');
  process.exit(1);
}
...
const res = await fetch(`https://www.googleapis.com/blogger/v3/blogs/${BLOGGER_BLOG_ID}/posts/?isDraft=false`, {
  method: "POST",
