...
**2026-09-18 업데이트: 영상은 중단하고 3D 스틸 캐러셀 10장으로 확정(사용자 지시).** 스틸은 gpt_image_2_5로 기준 단면도를 만든 뒤 이전 결과를 레퍼런스로 편집을 이어가는 방식이 해부 구조·일관성 모두 좋았다(장당 1크레딧). 텍스트만으로 seedance 영상을 뽑으면 뼈/치아를 혼동했다. 아래는 그 전 경위.
...
**⚠️ 2026-09-22 발견: 리포트 페이지가 거의 비어 있었다.** 주간 일지·워크스트림·요약표가 사라지고
불릿 2줄("새 건치의 하루 에피소드(10편) 기획 및 제작", "6편 재발행")만 남았으며 제목도 "…진행 리포트" →
"…리포트"로 바뀌어 있었다. 마지막 편집 2026-09-19T15:01Z(9/20 00:01 KST) — 루틴 22:00 슬롯 약 2시간 뒤라
...
- [x] 2026-09-22: 수동 글 "원주에서 치과 찾고 계신가요? 치과 고르는 기준과 원주 건강한치과 진료 안내"
      발행 (`scripts/wonju-dental-guide-post.html`, https://healthydentalwonju.blogspot.com/2026/09/blog-post_21.html).
      병원 기본정보·진료시간·FAQ + Dentist JSON-LD를 담은 로컬 SEO/GEO용 안내 글. 사실관계는 `index.html`과 대조 완료.
      Redis 토픽 "원주 치과 고르는 기준·병원 안내" 추가. 네이버 초안은 로그인 타임아웃으로 미완(아래 참고).
